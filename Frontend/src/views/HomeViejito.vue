<template>
  <div class="home">
    ...
    <v-container v-if="tasks.length > 0 && !loading" class="tasks-background" mt-4 id="Progress General">
      <v-row>
        <v-col cols="12">
          <v-card class="pa-3 white--text" color="blue darken-1" tile>
            Progreso de hoy:
            <v-progress-circular
              :value="progress"
              :size="100"
    ...
    <v-container v-if="tasks.length > 0 && !loading" id="Tasks Cards">
      <v-row>
        <v-col sm="6" lg="4" v-for="(task, index) of tasks" :key="index">
          <v-card
            ...
            dark
          >
            <v-card-title class="headline">{{ task.skill_title }}</v-card-title>
            <v-card-subtitle> {{ task.entity_title }} </v-card-subtitle>
            <v-card-subtitle>
              <v-progress-circular
